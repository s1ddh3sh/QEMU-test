#!/usr/bin/env bash
#
# dilithium_build.sh — compile every .ll file under a dilithium function's test
# directory into an ELF, then extract its qemu_witness.json.
#
# Usage:
#   ./dilithium_build.sh <directory>   # build just this one function directory
#   ./dilithium_build.sh               # build EVERY directory under tests_dilithium/
#
# Examples:
#   ./dilithium_build.sh tests_dilithium/pqcrystals_dilithium2_ref_basemul
#   ./dilithium_build.sh

DILITHIUM_RANDOMBYTES_OBJ="obj/common/dilithium_randombytes.c.o"
...
build_randombytes_obj() {
    mkdir -p "$(dirname "$DILITHIUM_RANDOMBYTES_OBJ")"
    arm-none-eabi-gcc \
    ...
    (
        ...
        for INPUT in "${LL_FILES[@]}"; do
            ...
            arm-none-eabi-objcopy \
                --redefine-sym randombytes=dilithium_llvm_randombytes \
                "${BUILD_DIR}/${FILENAME}.o"

            # Link object file into ELF
            arm-none-eabi-gcc \
                "${BUILD_DIR}/${FILENAME}.o" \
                "$DILITHIUM_RANDOMBYTES_OBJ" \
        ...
        python3 dist_tests/dilithium/setup/extract_qemu_witness.py "$LL_FILE"
    )
}

usage() {
    echo "Usage: $0 [directory]" >&2
    echo "  $0 tests_dilithium/pqcrystals_dilithium2_ref_basemul   # build just this one function" >&2
    echo "  $0                                                 # build every folder under tests_dilithium/" >&2
}
...
main() {
    ...
    if [ ! -d tests_dilithium ]; then
        echo "Error: tests_dilithium/ not found (run from the repo root, or pass" >&2
        echo "an explicit directory as before)." >&2
        exit 1
    fi

    DIRS=()
    for d in tests_dilithium/*/; do
        [ -d "$d" ] && DIRS+=("${d%/}")
    done

    if [ "${#DIRS[@]}" -eq 0 ]; then
        echo "Error: no subdirectories found under tests_dilithium/." >&2
        exit 1
    fi

    echo "[i] found ${#DIRS[@]} function directories under tests_dilithium/"
