...
# runs the single-background paired secret sweep (field_mod values x 2
# variants, e.g. 256 x 2 = 512 runs for a full-byte sweep) for use with a
# raw (s1,s2) pair comparison script, entirely IN-PROCESS via
# collect_dist_unicorn.py (dist_tests/common/unicorn_runner.py) -- no
# qemu-system-arm/gdb-multiarch subprocess is ever spawned.
#
# Same CLI/usage/argument-parsing/path derivation as collect_dist.sh; see
# that file's module docstring for the Dilithium-specific notes
# (--dilithium-mode, FIELD_MOD/--secret-pos semantics, byte-vs-coefficient
# offsets) -- unchanged here. --machine is still accepted for CLI
# compatibility but is ignored by collect_dist_unicorn.py (no real QEMU
# machine is launched by this backend).
#
# Usage:
...
FIXED_SCALARS=""
SEED=0
FIELD_MOD=""   # empty = not passed; collect_dist_unicorn.py auto-derives
               # it from secret_buf's declared distribution (sweep mode)
DILITHIUM_MODE=2
RECALIBRATE=0
...
echo "=== [2/2] collect_dist (unicorn): ${FUNC_NAME} / $(basename "$FAULTY_ELF") ==="
COLLECT_PY_ARGS=(
...
if [[ -n "$FIELD_MOD" ]]; then
    COLLECT_PY_ARGS+=(--field-mod "$FIELD_MOD")
fi
python3 "${SETUP_DIR}/collect_dist_unicorn.py" "${COLLECT_PY_ARGS[@]}"

echo "=== done: ${FUNC_NAME} / $(basename "$FAULTY_ELF") ==="
echo "[i] next: the ineffective / correction paired tests"
