...
# (correction_dilithium.py) for a single Dilithium function, against
...
#
# Example:
#   ./run_correction_paired.sh pqcrystals_dilithium2_ref_poly_add a
#
...
#   - --modulus defaults to 8380417 (Dilithium Q), not 3329. Unlike in
#     the ineffective test, this is load-bearing here: Delta is compared
#     as a VALUE, so a leftover 3329 would fold genuinely distinct
#     coefficient differences into one residue and hide real
#     disagreements.
...
#     clearly-announced HEURISTIC of word-size 4 / mod-sub; override
#     with --out-word-size/--diff-mode if it guesses wrong.
...
for faulty_elf in "${FAULTY_ELFS[@]}"; do
    ...
    python3 -u "${TEST_DIR}/correction_dilithium.py" \
