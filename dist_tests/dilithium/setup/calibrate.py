...
correct.elf's output. Writes active_lengths.json for use by the
paired-sweep / N-trial collection step.

Dilithium note: coefficients are int32_t, so every poly-shaped buffer is
twice its Kyber counterpart -- a poly is 1024 bytes, a mode-2 polyvec
(K = L = 4) is 4096, and an expanded matrix `polyvecl mat[K]` is 16384.
MAX_SAFE_PROBE_LEN below is raised accordingly so a legitimate matrix
buffer isn't capped by default; see its comment.
...
n-bounded loops over a poly/polyvec (poly_ntt, poly_add, polyvec_pointwise,
...) but can fail for non-contiguous access patterns (e.g. strided or
pointer-chain access, or a function that only touches part of a polyvec
buffer depending on a fixed_scalars-held dimension). Dilithium has a few
genuine offenders worth knowing about: poly_challenge reads a seed
non-uniformly, and the pack/unpack routines walk a buffer in
POLY*_PACKEDBYTES-sized strides. If a buffer's active length calibrates
to something implausible, fall back to a block-scan (partition into
chunks, probe each independently) instead of prefix binary search -- not
implemented here, flagged as a known limitation.

Note on distributions: calibration deliberately does NOT sample from a
buffer's declared "distribution" (uniform in [-ETA, ETA], an R_q domain,
a byte string, ...) -- it fills bytes uniformly at random in
[0, field_mod). The goal here is purely structural (which byte OFFSETS
influence the output), not producing realistic in-domain values, and a
raw uniform fill is simpler and just as effective for that. Realistic,
distribution-aware values are what collect_dist.py's data uses instead,
via driver_dist.py's collect mode.
...
Usage:
    python3 calibrate.py \
        ...
        --elf correct.elf --field-mod 256 --machine mps2-an386
"""
import argparse
import json
import os
...
SKIP_CALIBRATION_BELOW = 128  # buffers <= this size: just use full
                              ...
                              # Covers SEEDBYTES(32)/RNDBYTES(32) and
                              # CRHBYTES/TRBYTES(64); a single poly
                              # (1024 bytes) still gets calibrated.
...
#
# 16384 = K * L * N * 4 for mode 2, i.e. exactly one expanded matrix
# `polyvecl mat[K]` -- the largest buffer any Dilithium FUT legitimately
# takes. Anything above that is much more likely an over-declared or
# mismatched buffer than a real one.
MAX_SAFE_PROBE_LEN = 16384
...
def calibrate_buffer(elf_path, witness_path, func, field_mod, buf_name,
    ...
    # what an earlier version got wrong: `reference = probe(full_length,
    # base_seed)` used the raw declared length as its very first call,
    # with no guard, which is exactly what crashed QEMU on an oversized
    ...
    if full_length > max_probe_len:
        print(
            ...
            f"harness buffer sized for CRYPTO_SECRETKEYBYTES when this "
            ...
            f"genuinely large buffer). For reference, the largest "
            f"legitimate mode-2 buffer is the expanded matrix at "
            f"K*L*N*4 = 16384 bytes. Capping the calibration search to "
...
def main():
    ...
    ap.add_argument(
        ...
        help="Safety cap on the initial/reference probe length for any "
             "one buffer (default: %(default)s, one mode-2 expanded "
             "matrix). Declared alloca lengths beyond this are capped "
             "rather than probed directly, to avoid crashing "
             "memory-constrained QEMU targets on over-declared/MAX-sized "
             "or witness-mismatched buffers.",
    )
    ap.add_argument(
        "--fixed-scalars",
        default="",
        help="Comma-separated list of scalar layout names that must stay "
             ...
             "should be held fixed (e.g. a poly_uniform nonce, or a "
             "length/dimension argument).",
    )
    args = ap.parse_args()
    ...
    for name, s in layout.items():
        ...
        try:
            ...
            # Isolate the failure to this ONE buffer -- an exception here
            # must not kill the whole run, or a single oversized/
            # mismatched buffer means NO buffer for the function gets
            # calibrated, even ones that were perfectly fine.
            print(f"[!] {name}: calibration failed, skipping this buffer.\n{e}")
            failed_buffers[name] = str(e)
            continue
        active_lengths[name] = active_len
        if active_len % 4 != 0 and full_len % 4 == 0:
            # Not an error -- a genuine int32 buffer can be sensitive up
            # to the low byte of its last live coefficient -- but a
            # non-multiple-of-4 active length on a poly-shaped buffer is
            # worth eyeballing, since driver_dist.py samples 4 bytes per
            # coefficient and will truncate the final partial one.
            print(f"[!] {name}: active length {active_len} is not a "
                  f"multiple of 4 (int32 coefficient size); the last "
                  f"coefficient will be only partially filled.")
