#!/usr/bin/env python3
"""
ineffective_dilithium.py (single-background mode)
...
Dilithium-specific notes (vs. the Kyber version this was adapted from):
  ...
  - --out-word-size 4 (signed int32, little-endian) is the natural unit
    ...
    NOT Kyber's int16_t[N]. A mode-2 poly is 1024 bytes / 256 words and
    a polyveck/polyvecl is 4096 bytes / 1024 words. Raw byte-string
    buffers (signature, pk, sk, message, seeds, packed polynomials)
    should still use --out-word-size 1.
  ...
  - --active-len is in BYTES, matching calibrate.py's active_lengths.json
    ...
    internally (active_len // out_word_size) before indexing the decoded
    array. With int32 coefficients this division matters twice as much
    as it did for Kyber: a 4096-byte active length is 1024 words, not
    4096 and not 2048.

  - --diff-mode: "xor" (default -- fine for byte strings, and for
    equality-checking on R_q coefficients too, since xor(a,b)==0 iff
    a==b regardless of domain) or "mod-sub" ((y1 - y2) mod --modulus).
    For a pure ineffectiveness (equality) check either mode gives
    identical d1/d2, since both are exactly zero iff y1==y2.

  - sv filename padding is read straight out of each correct_sv match
    rather than re-formatted at a hardcoded width, so it works for any
    padding collect_dist.py used (3 digits at the default field_mod of
    256).

Usage:
    python3 ineffective_dilithium.py \
        --dist-dir tests_dilithium/pqcrystals_dilithium2_ref_poly_add/dist_paired \
        --out-buf c --active-len 1024 --out-word-size 4

    python3 ineffective_dilithium.py \
        --dist-dir tests_dilithium/pqcrystals_dilithium2_ref_verify/dist_paired \
...
def main():
    ...
    # active-len was calibrated against -- common in Dilithium, where a
    # polyvec input is 4x a poly output). load_sweep() already truncates
