...
# (ineffective_dilithium.py) for a single Dilithium function, against
# every already-collected single-background secret sweep for that
# function -- one sweep per faulty ELF variant found alongside the
# correct ELF.
#
# Usage:
...
# pqcrystals_dilithium2_ref_poly_add), matching both the ELF and the
# tests_dilithium/<func_name> witness directory -- see
# extract_qemu_witness.py and collect_dist.sh.
#
# Assumes trials already exist for each faulty ELF:
#   tests_dilithium/<func_name>/qemu_witness.json
#   tests_dilithium/<func_name>/active_lengths.json
#   tests_dilithium/<func_name>/<faulty_elf_stem>/dist_paired/{correct,faulty}_sv*.json
#
# Example:
#   ./run_ineffective_paired.sh pqcrystals_dilithium2_ref_poly_add a
#
# Dilithium-specific vs. the Kyber version:
#   - dist_tests/dilithium, tests_dilithium, build/tests_dilithium.
#   - the auto-derived word size for an R_q buffer is 4, not 2:
#     Dilithium's poly.coeffs is int32_t[N].
#   - --modulus defaults to 8380417 (Dilithium Q), not 3329.
#   - the poly-distribution list mirrors driver_dist.py's
#     _DISTRIBUTION_TABLE, including its normalization (case-folded,
#     whitespace-collapsed, trailing poly/polyvec/vector/matrix
#     qualifier stripped), so the two can't drift on spelling alone.
#   - many Dilithium outputs carry no declared "distribution" at all
#     (only traced INPUTS get one). When that happens and the buffer's
#     length is a whole number of 1024-byte polys, this falls back to a
#     clearly-announced HEURISTIC of word-size 4; override with
#     --out-word-size if it guesses wrong.
#   - --diff-mode/--modulus are accepted and forwarded for consistency
#     with run_correction_paired.sh, even though ineffective_dilithium.py's
#     result is identical either way -- it only ever checks delta==0, and
#     both modes agree exactly on when that holds.
...
# than intended. Cap the fallback explicitly. The cap is 4096 bytes = one
# mode-2 polyvec.
# ---------------------------------------------------------------------------
...
with open(witness_path) as f:
    layout = json.load(f)["layout"]
# "<name>_pre" is a legitimate --out-buf (an also_input buffer's pre-call
# snapshot, written into the trial JSON's "inputs" section) even though it
# is not a layout key in its own right.
base = requested[:-4] if requested.endswith("_pre") else requested
...
if requested.endswith("_pre"):
    if not layout[base].get("also_input"):
        print(f"[!] --out-buf '{requested}' asks for a pre-call snapshot, "
              f"but '{base}' is not marked also_input in the witness",
              file=sys.stderr)
        sys.exit(1)
elif layout[requested].get("role") != "output":
...
if secret_buf in active_lengths:
    length = active_lengths[secret_buf]
    if length == 0:
        print(
            f"[!] secret-buf '{secret_buf}' calibrated to an active length "
            f"of 0 -- the FUT appeared insensitive to every byte of it. "
            f"That is almost always a witness/binding problem (sample key "
            f"order vs. signature order, or a wrong declared length), not "
            f"a real result. Fix that before trusting anything downstream.",
            file=sys.stderr,
        )
        sys.exit(1)
    print(length)
...
# Auto-picks --out-word-size and --diff-mode for OUT_BUF. The normalized
# poly-distribution set below must stay in sync with driver_dist.py's
# _DISTRIBUTION_TABLE (every non-"bytes" entry).
derive_word_size_and_diff_mode() {
    python3 - "$WITNESS" "$OUT_BUF" <<'PYEOF'
import json, re, sys
witness_path, out_buf = sys.argv[1], sys.argv[2]

# Every non-byte-string distribution driver_dist.py knows about, i.e.
# everything sampled as int32 coefficients rather than raw bytes.
POLY_DISTRIBUTIONS = {
    "uniform in [-eta, eta]",
    "uniform in [-eta,eta]",
...
POLY_BYTES = 256 * 4  # one Dilithium poly: N int32 coefficients
...
def is_poly_dist(name):
    ...
    return key in POLY_DISTRIBUTIONS or QUALIFIER_RE.sub('', key) in POLY_DISTRIBUTIONS


with open(witness_path) as f:
    layout = json.load(f)["layout"]

base = out_buf[:-4] if out_buf.endswith("_pre") else out_buf
spec = layout[base]

# For a "<name>_pre" snapshot the relevant domain is the INPUT one, which
# is what "distribution" holds; for the post-call value it is
# "output_distribution" when extract_qemu_witness.py recorded one
# (in-place functions change domain across the call), else "distribution".
if out_buf.endswith("_pre"):
    dist = spec.get("distribution")
else:
    dist = spec.get("output_distribution") or spec.get("distribution")

if spec.get("type") == "scalar":
    length = spec.get("length", 1)
    word_size = length if length in (1, 2, 4) else 1
    diff_mode = "xor"
elif is_poly_dist(dist):
    word_size = 4
    diff_mode = "mod-sub"
elif dist is None and spec.get("length", 0) >= POLY_BYTES and \
        spec.get("length", 0) % POLY_BYTES == 0:
    # No declared distribution -- normal for an output buffer, since only
    # traced arguments get a TRACE_DISTRIBUTION. Its length being a whole
    # number of polys is a strong hint it holds int32 coefficients, but it
    # IS a guess: a packed byte string could coincidentally be a multiple
    # of 1024 too.
    word_size = 4
    diff_mode = "mod-sub"
    print(
        f"[!] '{out_buf}' has no declared distribution; its length "
        f"({spec.get('length')}) is a whole number of 1024-byte polys, so "
        f"guessing word-size 4 / diff-mode mod-sub. Pass --out-word-size "
        f"1 --diff-mode xor if it is really a packed byte string.",
        file=sys.stderr,
    )
else:
    word_size = 1
...
for faulty_elf in "${FAULTY_ELFS[@]}"; do
    ...
    python3 -u "${TEST_DIR}/ineffective_dilithium.py" \
