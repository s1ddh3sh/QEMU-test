...
fill, so the background stays a realistic Kyber-shaped input; only the
swept byte itself is forced to an explicit value.
...
test across multiple backgrounds, run this with --n-seeds worth of
different --seed values and test the resulting per-seed distributions
downstream; single-seed output here is meant to be read directly.

Usage:
    python3 collect_dist.py --witness tests_kyber/poly_ntt/qemu_witness.json \
        --active-lengths tests_kyber/poly_ntt/active_lengths.json \
        --correct-elf correct.elf --faulty-elf faulty.elf \
        --func poly_ntt --field-mod 256 --kyber-k 3 \
        --secret-buf r --secret-pos 0 \
...
def main():
    ...
    ap.add_argument(
        "--kyber-k", type=int, default=3, choices=(2, 3, 4),
        help="Kyber security parameter K, used only to pick the correct "
    ...
    print(f"[i] sweeping {args.secret_buf}[{args.secret_pos}] over "
          ...
          f"(shared across the whole sweep), K={args.kyber_k}")
