...
n-bounded loops over a poly/polyvec (poly_ntt, poly_add, indcpa_enc's
message/coins, ...) but can fail for non-contiguous access patterns (e.g.
strided or pointer-chain access, or a function that only touches part of
a polyvec buffer depending on a fixed_scalars-held dimension). If a
buffer's active length calibrates to something implausible, fall back to
a block-scan (partition into chunks, probe each independently) instead
of prefix binary search -- not implemented here, flagged as a known
limitation.

Note on distributions: calibration deliberately does NOT sample from a
buffer's declared "distribution" (centered binomial, R_q coefficient
domain, byte string, ...) -- it fills bytes uniformly at random in
...
distribution-aware values are what collect_dist.py's N-trial data uses
instead, via driver_dist.py's collect mode.
...
Usage:
    python3 calibrate.py --witness tests_kyber/poly_ntt/qemu_witness.json \
