...
# raw (s1,s2) pair comparison script (e.g. ineffective_paired_test.py),
# entirely IN-PROCESS via collect_dist_unicorn.py
...
# that file's module docstring for the Kyber-specific notes (--kyber-k,
# FIELD_MOD/distribution-driven background sampling) -- unchanged here.
# --machine is still accepted for CLI compatibility but is ignored by
# collect_dist_unicorn.py (no real QEMU machine is launched by this
# backend).
#
# Usage:
...
FIELD_MOD=""   # empty = not passed; collect_dist_unicorn.py auto-derives
               # it from secret_buf's declared distribution
KYBER_K=3
...
COLLECT_PY_ARGS=(
    ...
    --func "$FUNC_NAME" --kyber-k "$KYBER_K"
