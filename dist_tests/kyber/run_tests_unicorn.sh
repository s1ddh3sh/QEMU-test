...
# paired-sweep pipeline for one Kyber function, with calibration and
...
# up automatically from <repo_root>/kyber.json (keyed by func_name); an
...
#
# Example:
#   ./run_tests_unicorn.sh pqcrystals_kyber768_ref_poly_cbd_eta1
#   ./run_tests_unicorn.sh pqcrystals_kyber768_ref_poly_cbd_eta1 --secret-buf buf --secret-pos 0
...
echo "[i] secret buf:    ${SECRET_BUF} (pos ${SECRET_POS}), field-mod ${FIELD_MOD:-<auto-derived from secret-buf distribution>}"
...
echo "[i] per-faulty-ELF results under: tests_kyber/${FUNC_NAME}/<faulty_elf_stem>/{ineffective,correction}_paired_result.txt"
