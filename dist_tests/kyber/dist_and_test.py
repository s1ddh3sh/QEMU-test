#!/usr/bin/env python3
"""
early_stop_sweep.py — incrementally collect (or reuse already-collected)
correct/faulty trials for increasing secret values sv = 0, 1, 2, ..., and
STOP as soon as the requested test(s) (ineffective and/or correction) have
found at least one witness -- i.e. an (s1, s2) pair with d1 != d2
(ineffective) or Delta(s1) != Delta(s2) (correction), at ANY output
position.
...
earlier full sweep), they are loaded from disk instead of re-launching
QEMU. Pointing this at an already-fully-collected dist_paired directory
...
used from the start -- with zero new QEMU boots.

Expected directory layout (matching collect_dist.sh / run_ineffective_paired.sh):
    dist_tests/kyber/setup/collect_dist.py
    dist_tests/kyber/ineffective_kyber.py
    dist_tests/kyber/early_stop_sweep.py   <- this file
...
Usage:
    ...
    # to see how early stopping WOULD have kicked in:
    python3 early_stop_sweep.py \
...
from collect_dist import run_one, RunFailed  # noqa: E402  (unmodified)
...
def main():
    ...
    ap.add_argument(
        ...
        help="word size (in bytes) the override at --secret-pos is "
             ...
             "GDB_DRIVER_OVERRIDE_WORD_SIZE / collect_dist.py's run_one() "
             "'word_size' parameter. 1: byte-oriented override (the "
             "original behavior). 2: --secret-pos is a COEFFICIENT/word "
             "index, not a byte offset, and the override value is "
             "encoded as a signed int16_t -- use this for an R_q-shaped "
             "secret buffer (Kyber poly.coeffs) so a full coefficient is "
             "forced to an explicit value rather than just its low byte. "
             "Default: %(default)s",
    )
